...
const createRecipeObject = function(data) {
    ...
    return {
        ...
        //if key exist that is the same as key: recipe.key,
        //if not exist than doesn't happened
        ...recipe.key && {
            key: recipe.key
        }
    };
...
const loadRecipe = async function(id) {
    try {
        ...
        //bookmarked still fill after the click to other recipe
...
const persistBookmarks = function() {
    //Stored Recipe by localStorage
    localStorage.setItem("bookmarks", JSON.stringify(state.bookmarks));
};
const addBookmark = function(recipe) {
    ////Method that ADD fill bookmark
    //Add bookmark
...
const deleteBookmark = function(id) {
    //Method that REMOVE fill bookmarked
    //Delete bookmark
...
const uploadRecipe = async function(newRecipe) {
    try {
        ...
        //filter out only data that we interested in, without empty data
        const ingredients = Object.entries(newRecipe).filter((entry)=>entry[0].startsWith("ingredient") && entry[1] !== "").map((ing)=>{
            const ingArr = ing[1].split(",").map((el)=>el.trim());
            //ing[1] - the second element of Array!!!
            // const ingArr = ing[1].replaceAll(' ', '').split(',');
            if (ingArr.length !== 3) throw new Error("Wrong ingredient format! Please use the correct format :) ");
            const [quantity, unit, description] = ingArr;
            //if we don't have a quantity--- quantity: quantity ? +quantity: null
            return {
...
const KEY = "3f6c0d46-3acd-4b13-9066-b8b2b6441e00";
...
class PaginationView extends (0, _viewJsDefault.default) {
    ...
    _generateMarkup() {
        ...
        if (curPage === 1 && numPages > 1) return generateMarkupMethod.next;
        //Last page
        if (curPage === numPages && numPages > 1) return generateMarkupMethod.previous;
        //Other page
        if (curPage < numPages) return generateMarkupMethod.previous + generateMarkupMethod.next;
